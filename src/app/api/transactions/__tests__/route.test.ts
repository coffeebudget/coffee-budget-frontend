...
describe('/api/transactions', () => {
  ...
  describe('GET /api/transactions', () => {
    ...
    it('should forward query parameters to backend', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/transactions?category=groceries&limit=10');
      await GET(request);

      expect(global.fetch).toHaveBeenCalledWith(
        'http://localhost:3001/transactions?category=groceries&limit=10',
        expect.any(Object)
      );
    });
  ...
  describe('POST /api/transactions', () => {
    ...
    it('should handle network errors on creation', async () => {
      mockGetServerSession.mockResolvedValue({
        user: { 
          id: '1', 
          email: 'test@example.com',
